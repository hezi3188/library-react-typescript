...
const useAddBookDialog = (
  ...
  const addBook = (bookId: number) => {
    addBookToUserDb({
      ...
      handleClose();
      let newObj: Book = {
        id: obj.data.createBooksList.bookByBookId.id,
        name: obj.data.createBooksList.bookByBookId.name,
        authorId: obj.data.createBooksList.bookByBookId.authorByAuthorId.id,
      };
